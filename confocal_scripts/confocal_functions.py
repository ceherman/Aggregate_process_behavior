...
def read_image(file, gamma=1, epsilon=0, scan='xz'):
    ...
    shape = np.shape(data)
    new = np.zeros((shape[1], shape[2], n_color))
    ...
    # Pad with zeros if there are only two colors
    if np.shape(new)[2] == 2:
        new = np.pad(new, [(0, 0), (0, 0), (0, 1)], mode='constant')
    return new
...
def plot_color_montage(aspect, x, new, n_colors=3, scale_bar=True):
    ...
    fig, ax = plt.subplots(n_colors+1, 1, figsize=(n*aspect, n*n_colors))
...
def plot_greyscale_montage(aspect, x, new, n_colors=3, scale_bar=True):
    """Order:  large, small, mAb"""
    n = 3 # scales the image size in jupyter - ultimately only matters relative to the scale bar text size
    fig, ax = plt.subplots(n_colors, 1, figsize=(n*aspect, n*n_colors))
    plt.subplots_adjust(hspace=0)
